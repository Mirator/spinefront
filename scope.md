...
## High-level goal
Spinefront is a compact, wave-based cooperative action-builder. The player defends a central keep across a sequence of short day/night cycles, using mobility, melee combat, automated defenses, and shrine tech to hold a thin frontline. Runs are short (minutes), tuned for iteration and GitHub Pages delivery, and survival hinges on managing aura stability against the weight of carried gold.
...
## Player kit
- **Movement:** Grounded side-scroller with horizontal acceleration (220 u/s walk, 340 u/s sprint) and a single jump (650 force). Gravity is high (1800) to keep arcs tight; world clamps prevent leaving the play lane. Sprinting efficiency decays as gold burden rises.
...
- **Aura (life shield):** Starts at 100. Taking aura damage deals 38 loss, applies short immunity (0.65s), delays recovery (1.1s), and drops up to 3 gold. Aura recovers only when you hold at least 4 gold **and** stand within owned territory (around the walls), ticking at 12 per second, and is boosted or hindered by relic modifiers. Hitting 0 aura pushes the player into a critical state; the next hit in that state knocks them from the island.
- **Gold burden:** Carrying gold slows sprint speed and increases aura instability. Excess gold raises recovery delays and can erode aura until the burden is reduced by spending or dropping gold (Interact+Down).

## Economy & interaction
- **Income:** +10 gold each dawn; +5 gold at the start of night. Island modifiers may add flat bonuses. Gold is both fuel and burden: hoarding slows sprinting and destabilizes aura recovery, encouraging timely spending or manual drops.
...
- **Barricades and drops:** During daytime, interact + down to place a temporary barricade at 7 gold (36×28 footprint, 70 HP, cannot overlap structures). Holding interact + down away from structures drops 5 gold to shed burden and stabilize aura.
...
- **Targeting:** Enemies prioritize nearby living structures; if none are close, they continue forward. Damage applies to walls/towers or the player (causing aura hits and gold spills).
...
- **Loss states:**
  - Aura collapse: reach 0 aura and take another hit while critical.
  - (Secondary) Aura instability can erode reserves if gold burden is extreme.
- **Victory/ascension:** Survive 3 nights without falling to trigger an ascension prompt. Accepting ascension resets the run on the next island while preserving learned shrine unlocks, legacy, and updating island history.

## Exploration: jump puzzles
- Each island seeds a handful of visible, optional vertical jump puzzles off the main lane.
- Puzzles are enemy-free but consume precious daytime; failing one wastes additional daylight, risking a night transition mid-climb.
- Completing a puzzle grants a single reward choice:
  - **Gold:** Immediate currency with burden risk.
  - **Relics:** Run-limited modifiers (e.g., aura recovery boost, burden resistance).
  - **Legacy:** Meta-progression currency that persists across runs.
- Rewards are intentionally capped; puzzles trade time for utility without granting compounding combat power.
...
- **HUD:** Displays gold/burden readouts, shrine tech selection, wave/night info, aura status, puzzle reminders, and contextual HUD text. Menu panel lists objectives, controls, and island bonus, with a start/resume button. Outcome overlay communicates victory/defeat and restart options.
